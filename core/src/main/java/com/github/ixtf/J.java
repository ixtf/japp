...
public class J {
  ...
  public static LocalDate localDate(Date date) {
    return ofNullable(date).map(J::localDateTime).map(LocalDateTime::toLocalDate).orElseThrow();
  }

  public static LocalTime localTime(Date date) {
    return ofNullable(date).map(J::localDateTime).map(LocalDateTime::toLocalTime).orElseThrow();
  }
  ...
  public static Date date(LocalDate ld) {
    return ofNullable(ld).map(LocalDate::atStartOfDay).map(J::date).orElseThrow();
  }

  public static Date date(LocalTime lt) {
    return ofNullable(lt).map(it -> it.atDate(LocalDate.now())).map(J::date).orElseThrow();
  }
  ...
  public static Collection<File> listFiles(
      final String directory, final String[] extensions, final boolean recursive) {
